...
int main(int argc, char * argv[]) {      
  ...
  //    //    counter_zeros += 1;
  //}  
  for(int i = 2; i < partial_kernel_x_size-2; i++) {
        for(int j = 0; j < size_in_y-4; j++) {
            for(int k = 0; k < size_in_z; k++) {
                if(j < size_in_y-4-1 || (j == size_in_y-4-1 && k < size_in_z - 6)) {
                    int idx = i * size_in_y * size_in_z + j * size_in_z + k;
                    printf("%lf\n", (sw_data[0][idx]));
                }
            }
        }
